...
class DatabaseManager(TradingLoggerMixin):
    ...
    async def get_recent_analyses(self, limit: int = 10) -> List[Dict]:
        """Get the most recent market analyses."""
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("""
                SELECT market_id, decision_action, confidence, analysis_timestamp 
    ...
    async def close_position_with_pnl(self, position: 'Position', exit_price: float) -> None:
        """
        Close a position, log PnL, and record a trade log entry.

        Args:
            position: The Position object to close.
            exit_price: The exit price (in dollar terms, 0.0-1.0).
        """
        # Calculate PnL — exit_price and entry_price are both in contract-value terms (0-1)
        pnl = (exit_price - position.entry_price) * position.quantity

        # Close the position
        await self.update_position_status(position.id, "closed")
        ...
        await self.add_trade_log(trade_log)

        self.logger.info(
            f"Closed position {position.market_id} ({position.side}): "
    ...
    async def was_recently_analyzed_smart(self, market_id: str, skip_hours: float = 0.5, trade_hours: float = 2.0) -> bool:
        """Check if market was analyzed recently, with shorter cooldown for SKIP decisions.

        SKIP markets are re-eligible after skip_hours (default 30 min).
        YES/NO traded markets stay blocked for trade_hours (default 2h).
        """
        async with aiosqlite.connect(self.db_path) as db:
            # Check for any YES/NO decision within trade_hours
            trade_cutoff = (datetime.now() - timedelta(hours=trade_hours)).isoformat()
            cursor = await db.execute("""
                SELECT COUNT(*) FROM market_analyses
                WHERE market_id = ? AND analysis_timestamp > ? AND decision_action IN ('YES', 'NO')
            """, (market_id, trade_cutoff))
            if (await cursor.fetchone())[0] > 0:
                return True

            # Check for any SKIP decision within skip_hours
            skip_cutoff = (datetime.now() - timedelta(hours=skip_hours)).isoformat()
            cursor = await db.execute("""
                SELECT COUNT(*) FROM market_analyses
                WHERE market_id = ? AND analysis_timestamp > ? AND decision_action = 'SKIP'
            """, (market_id, skip_cutoff))
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            for row in rows:
                position = Position(
                    ...
                    target_confidence_change=row["target_confidence_change"],
                )
